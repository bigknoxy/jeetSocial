...
class KindnessManager {
    ...
    setupWebSocketListeners() {
        ...
        const unsubscribeKindnessUpdate = wsService.onKindnessUpdate((event) => {
    ...
    setupCrossTabSync() {
        ...
        window.addEventListener('storage', (event) => {
            try {
                ...
                if (!event.key || event.key !== 'jeet_kindness_update')
                    return;
                ...
                // Ignore older events
                if (payload.ts <= this._lastAppliedKindnessTs)
                    return;
    ...
    async ensureToken(postId) {
        ...
        try {
            ...
            if (ssToken)
                this.token = ssToken;
            if (ssExpiry)
                this.tokenExpiry = parseInt(ssExpiry, 10);
        }
        catch (err) {
    ...
    async awardKindness(postId, buttonElement) {
        ...
        if (countElement) {
            ...
            if (liveRegion)
                liveRegion.textContent = `Kindness Given for post ${postId}. New count ${originalCount + 1}`;
...
//# sourceMappingURL=KindnessManager.js.map