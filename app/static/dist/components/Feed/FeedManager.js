...
class FeedManager {
    ...
    setupWebSocket() {
        ...
        wsService.on('connection_status_change', (status) => {
    ...
    handleNewPost(postData) {
        ...
        const existingIds = currentPosts.map((p) => p.id);
     ...
     */
    insertNewPostInDOM(post) {
        const feed = document.getElementById('feed');
        if (!feed)
            return;
        const existingIds = Array.from(feed.children).map(node => node.dataset?.id);
    ...
    `;
        try {
            ...
            try {
                console.debug('[FetchFeed] /api/posts payload', posts);
            }
            catch { /* ignore */ }
    ...
    async pollForNewPosts() {
        ...
        try {
            ...
            const newPosts = latestPosts.filter((post) => {
                ...
                return postTime > lastTimestamp && !state.feed.posts.some((existing) => existing.id === post.id);
            });
            // Process new posts
