...
class AdminDashboard {
    ...
    async loadRecentActivity() {
        try {
            const response = await fetch('/admin/reports?limit=10', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${this.authClient.getToken()}`
                }
    ...
    async loadChartData() {
        try {
            const [reportsTrend, categories, actions] = await Promise.all([
                this.fetchChartData('/admin/reports/queue'),
                this.fetchChartData('/admin/reports'),
                this.fetchChartData('/admin/stats')
            ]);
